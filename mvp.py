...
DECISION_LOG_PATH = os.path.join(LOGS_DIR, "decision_log.jsonl")
...
ESCALATION_EMAILS = ["controller@metro.com"]

ROLE_PROFILES: Dict[str, Dict[str, Any]] = {
    "Engineering": {
        "focus": [
            "specification", "standard", "drawing", "maintenance", "downtime",
            "throughput", "SOP", "RCA", "root cause", "inspection"
        ],
        "prompt": (
            "Summarize for engineering: emphasize technical details, procedures, "
            "standards, deviations, and required actions."
        ),
        "table_priority": ["spec", "part", "bom", "schedule", "inspection"],
    },
    "Finance": {
        "focus": [
            "cost", "price", "budget", "invoice", "capex", "opex", "amount",
            "tax", "gst", "penalty", "saving", "revenue"
        ],
        "prompt": (
            "Summarize for finance: extract monetary amounts, variances, due dates, "
            "cost drivers, and approvals required."
        ),
        "table_priority": ["financial", "cost", "price", "budget", "invoice", "amount"],
    },
    "Safety": {
        "focus": [
            "incident", "hazard", "unsafe", "near miss", "ppe", "lockout",
            "danger", "risk", "compliance", "violation", "emergency"
        ],
        "prompt": (
            "Summarize for safety: highlight incidents, risks, mitigations, "
    ...
    "HR": {
        "focus": [
            "policy", "leave", "holiday", "recruitment", "training", "benefit",
            "disciplinary", "overtime", "attendance", "payroll"
        ],
        "prompt": (
            "Summarize for HR: focus on policies, people actions, trainings, "
    ...
    "Management": {
        "focus": [
            "summary", "risk", "impact", "timeline", "decision",
            "escalation", "okrs", "kpi", "milestone"
        ],
        "prompt": (
            "Summarize for top management: provide an executive summary with key risks, "
            "decisions, timelines, and owners."
        ),
        "table_priority": ["kpi", "milestone", "timeline", "budget"],
    },
}
...
def table_to_text(table_rows: List[List[str]]) -> str:
    ...
    for r in table_rows[1:6]:
        pairs = [f"{str(h).strip()}: {str(c).strip()}" for h, c in zip(header, r)]
        out.append("; ".join(pairs))
...
def classify_priority(text: str) -> str:
    ...
    high_words = [
        "accident", "hazard", "failure", "emergency", "risk",
        "unsafe", "violation", "fire", "breakdown", "critical"
    ]
    medium_words = [
        "delay", "maintenance", "issue", "inspection",
        "repair", "fault", "warning", "deviation"
    ]

    if any(word in txt for word in high_words):
        return "HIGH"
    if any(word in txt for word in medium_words):
        return "MEDIUM"
    return "LOW"


def compute_confidence_from_hits(hits: List[Dict[str, Any]]) -> int:
    if not hits:
        return 0
    return min(100, 40 + len(hits) * 10)


def validate_answer(ans: str) -> bool:
    txt = (ans or "").strip().lower()
    if len(txt) < 40:
        return False
    if "no relevant information found" in txt:
        return False
    if "couldn’t find relevant evidence" in txt:
        return False
    return True


def get_recipients(role: str, priority: str) -> List[str]:
    base = list(STAKEHOLDER_EMAILS.get(role, []))
    if priority == "HIGH":
        for mail in ESCALATION_EMAILS:
            if mail not in base:
                base.append(mail)
    return base
...
def build_email_subject(file_path: str, role: str, priority: str) -> str:
    return f"[{priority}] Snapshot - {os.path.basename(file_path)} - {role}"
...
def extract_pdf_structured(file_path: str) -> List[Dict[str, Any]]:
    ...
    if HAS_CAMELOT:
        try:
            tables = camelot.io.read_pdf(file_path, pages="all", flavor="lattice")
            for t in tables:
                page_num = t.page if isinstance(t.page, int) else 0
                camelot_tables_by_page.setdefault(page_num, []).append(t.df.values.tolist())
        except Exception:
            camelot_tables_by_page = {}
...
def build_role_context(structured_pages: List[Dict[str, Any]], role: str) -> str:
    ...
    for p in structured_pages:
        ...
        for line in t.split("\n"):
            line_l = line.lower()
            if (
                any(ft in line_l for ft in focus_terms)
                or len(line) < 80
                or re.match(r"^[A-Z0-9][A-Za-z0-9 .:/_-]{0,60}$", line)
            ):
    ...
    for p in structured_pages:
        for t in p.get("tables", []):
            rows = t.get("rows", [])
            header_join = ",".join([str(c).lower() for c in (rows[0] if rows else [])])
            score = sum(1 for tag in pri_tags if tag in header_join)
            if role == "Finance" or score > 0:
                table_snippets.append(table_to_text(rows))
...
# =============================================================================
# Indexing + Retrieval for Query Layer
# =============================================================================
def build_chunks(structured_pages: List[Dict[str, Any]], max_chars: int = 900) -> List[Dict[str, Any]]:
    chunks: List[Dict[str, Any]] = []
...
def answer_query(query: str, chunks: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # self-correction: retry with relaxed query
    if not hits:
        relaxed_query = " ".join(query.split()[:2]).strip()
        if relaxed_query:
            hits = retrieve_chunks(relaxed_query, chunks, top_k=6)
    ...
    if not validate_answer(ans):
        ans = "Low confidence answer. Needs review."
...
def make_snapshot_docx(
    ...
    d.add_heading(f"{stakeholder} Snapshot", 0)
    d.add_paragraph(f"Source: {os.path.basename(source_path)}")
    d.add_paragraph(f"Priority: {priority}")
    d.add_paragraph(f"Confidence: {confidence}%")
    d.add_paragraph("")
...
def process_file(file_path: str) -> List[Tuple[str, str]]:
    ...
    for role in ["Engineering", "Finance", "Safety", "HR", "Management"]:
        ...
        priority = classify_priority(role_sum)
        confidence = compute_confidence_from_hits(chunks[:6])

        docx_path = make_snapshot_docx(
        ...
        recipients = get_recipients(role, priority)
        ...
        log_decision({
            ...
            "structured_json": os.path.basename(struct_path),
            "index_file": os.path.basename(idx_path),
            "snapshot_file": os.path.basename(docx_path)
        })

        try:
            smtp.send_email(
                subject=build_email_subject(file_path, role, priority),
                body=(
                f"Attached is your role-specific snapshot.\n\n"
                f"Priority: {priority}\n"
                ...
                recipients=recipients,
                attachments=[docx_path],
                priority=priority   # 🔥 THIS LINE IS MANDATORY
            )
